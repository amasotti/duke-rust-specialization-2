...
#[clap(
    version = "1.0",
    author = "Noah Gift, Antonio Masotti (update to new crate versions)",
...
fn main() {
    ...
    match args.command {
        ...
        Some(Commands::Sort {
            ...
            let df2: DataFrame = df2.drop_nulls::<&str>(None).unwrap();
            //sort the dataframe by the year column and by order passed in
            // if order is true, sort in ascending order, else sort in descending order
